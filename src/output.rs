...
pub fn print(info: Info, path_to_ascii: String) {
    let mut ascii_art: Vec<ColoredString> = vec![
        "⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⢸⣿⣿⣷⣜⢿⣧⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⡄⠻⣿⣿⣿⣿⣦⠄⠄".to_string().blue(),
        "⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡇⣿⣿⣿⣿⣮⡻⣷⡙⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⣿⣿⣆⠙⣿⣿⣿⣿⣧⠄".to_string().blue(),
        "⣿⣿⣿⣿⣿⣿⣿⣿⣿⠏⣿⣿⣿⣿⣿⣿⣧⢸⣿⣿⣿⡘⢿⣮⡛⣷⡙⢿⣿⡏⢻⣿⣿⣿⣧⠙⢿⣿⣿⣷⠘⢿⣿⣆⢿⣿⣿⣿⣿⣆".to_string().blue(),
        "⣿⣿⣿⣿⣿⣿⣿⣿⡿⠐⣿⣿⣿⣿⣿⣿⠃⠄⢣⠻⣿⣧⠄⠙⢷⡀⠙⢦⡙⢿⡄⠹⣿⣿⣿⣇⠄⠻⣿⣿⣇⠈⢻⣿⡎⢿⣿⣿⣿⣿".to_string().blue(),
    ...
    if !path_to_ascii.is_empty() {
        match std::fs::exists(&path_to_ascii) {
            Ok(res) => {
                ...
                let lines = std::fs::File::open(path_to_ascii)
                    .unwrap();

                ascii_art= std::io::BufReader::new(lines)
                    .lines()
                    .map(|line| line.unwrap().blue())
                    .collect();
            },
            Err(_) => {
    ...
    for i in 0..max_lines {
        println!("{:<30} {}", ascii_art.get(i).unwrap_or(&"".to_string().blue()), sys_info.get(i).unwrap_or(&"".to_string()));
    }
}
